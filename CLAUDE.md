...
- Terrain tiling layout:
  ...
  - Global base (offline-built, low-zoom planet coverage): `downloads/terrain/base_z8/` served at `/terrain/base/...`. It predates vertex normals (`extensions: []`), so with lighting on the area it covers renders as a near-black wedge — fixing that means re-tiling the base.
