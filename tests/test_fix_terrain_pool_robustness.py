...
(src/services/terrain_tiling/cesium_terrain.py, 范本 src/services/contour_engine.py):
...
class _InProcessPool:
    ...
    cesium_terrain 的注释),替身漏了这个参数就会以 TypeError 把无关用例带红 ——
...
def test_worker_tile_returns_none_on_bad_read(tmp_path):
    ...
    try:
        ...
        # with_normals 是法线接线时加的（按生产默认传 True）；这里走 'grid' 让
        # 本条测试只盯坏块容错，不受三角化/择优分支影响。
        result = ct._worker_tile(
            (0, 0, 0, -180.0, -90.0, 0.0, 0.0, 17, str(tmp_path), "grid", 0.15, True))
    finally:
