...
that gate the cache behavior: _try_promote_from_cache and _link_or_copy.
"""
...
def test_promote_from_cache_copy_fallback_leaves_no_part_file(tmp_path, engine, monkeypatch):
    """跨盘（os.link 不可用）时提升走「同目录临时件 + 原子 replace」，
    临时件不得残留 —— 否则任务目录里会长期挂着 `*.part.<pid>.<id>`。"""
    cache_dir = tmp_path / "cache"
    cache_dir.mkdir()
    (cache_dir / GRANULE).write_bytes(PAYLOAD)
    dest = tmp_path / "task" / GRANULE
