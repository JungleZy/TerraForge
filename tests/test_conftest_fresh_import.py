...
fresh_import 对 dotted 模块(如 src.core.database)做 pop + 重导入时,importlib
...
def test_fresh_import_restores_parent_package_attribute():
    import src.core
    import src.core.database  # noqa: F401  (确保父包属性已建立)

    from conftest import fresh_import

    original = sys.modules['src.core.database']
    assert src.core.database is original

    with pytest.MonkeyPatch.context() as mp:
        fresh = fresh_import(mp, 'src.core.database')
        assert fresh is not original
        assert sys.modules['src.core.database'] is fresh
        assert src.core.database is fresh, '重导入期间父包属性应指向新实例'

    assert sys.modules['src.core.database'] is original, 'teardown 后 sys.modules 必须恢复'
    assert src.core.database is original, 'teardown 后父包属性必须恢复原实例(否则 split-brain)'
