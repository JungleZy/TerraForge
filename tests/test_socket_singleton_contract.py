...
def _strip_js_comments(src):
    ...
    要么被字符串里的引号带偏。本项目的 js 三种坑都真实存在，缺一条就会被带偏：
      - 字符串里的 `//`（history.js 的 `//${host}/vt?...`）；
      - 含裸引号的正则字面量（ui.js:233 的 `.replace(/"/g, '&quot;')`）；
      - **嵌套模板串**（history.js:282 在 `${}` 里又开了一个反引号串）。
