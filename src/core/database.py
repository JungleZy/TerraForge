...
DEFAULT_CONFIGS = [
    ...
    ('max_gdal_slots', '2'),
    # --- 磁盘预算（§4.2）。只估算与展示，不拦截：2026-08 起估算超过可用空间
    # 既不拒绝启动也不中途叫停，判决的去处是前端「需要约 X、可用 Y」与任务日志
    # 里的一行数字。---
    # 总是给系统留下的余量（MB）。GeoD 用固定 512 MiB 地板且不随剩余工作量
    # 重估（fs_util.rs:57-67），报不出「这一层还要 40 GB、只剩 3 GB」——
    # 这里的地板只负责「别把盘写死」的提示，够不够跑完由估算器按剩余工作量回答。
    ('disk_reserve_mb', '2048'),
    # 估算值的安全系数（GeoD 用 1.15，同一量级）。估算永远会错，问题只是
    # 错多少；系数把「错一点」吸收掉，「错 17 倍」由分档估算解决而不是靠它。
    ('disk_safety_factor', '1.15'),
    # 磁盘预算展示开关。关掉后 verdict 恒 ok：前端不弹「磁盘不足」、日志措辞
    # 变成「检查已关」，估算数字照常给。它从来就不是（现在更不是）一道闸门。
