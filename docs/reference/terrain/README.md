...
| [`global-base-build.md`](global-base-build.md) | 全球低层级基础地形（`downloads/terrain/base_z8/`）的完整构建流程：自备全球 DEM 的前置成本、可直接跑的命令与完整参数表、`--max-level` 省略后果的实算量级、以及输出目录与 `terrain_global_base_path` 的对齐要求 |
