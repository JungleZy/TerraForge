...
> **状态（底图随任务植入之后）：默认路径上已消解，兜底路径上依旧。** 底图可用时任务只切 z8 起的层级（tiler 恒传 `min_level = 8`，`build_terrain` 再把它钳到实际 `max_level` 以下 —— `maxzoom < 8` 或负档位偏移时起切层级就是那个实际值），z0–z4 那 682 片垃圾瓦片根本不再生成，低层级直接由植入进任务目录的底图瓦片提供，也不再有 `parentUrl`、没有级联可言。下面描述的整套现象只在**底图不可用**（有人删了 `assets/terrain/*.part`）退回 parentUrl 级联时才成立 —— 那条路径一行未动，所以原文保留。
