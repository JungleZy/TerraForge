...
| 目录 | 放什么 | 什么时候看它 |
|---|---|---|
| [`guides/`](guides/) | 照着做的操作指南（4 份）：快速上手、安装、构建打包、成品分发 | 要装环境、要跑起来、要打包发版、拿到 zip 包不知道怎么用 |
| [`reference/`](reference/) | 当前实现说明：计时系统、DOM 局部更新、`terrain/` 地形加载与全球底图构建 | 想知道某个已上线机制现在是怎么跑的 |
| [`notes/`](notes/) | 调研笔记与**尚未实施**的设计稿（2 份：共享缓存清理方案、GeoLibre 调研） | 找 backlog、评估要不要做某件事 |
| [`reviews/`](reviews/) | 带日期的时点审查快照（6 份 + 1 个流产审查目录） | 想知道某个决策/取舍的来龙去脉，或某轮审查发现了什么 |
| [`archive/`](archive/) | 历史文档归档（10 份，全部 2026-05 ~ 07） | 考古：某个旧做法为什么被推翻 |
| [`superpowers/`](superpowers/) | 历史实施计划 `plans/`（10 份）与配对设计稿 `specs/`（11 份） | 考古：某个功能当初是怎么设计和排期的 |
...
- `notes/` —— 里面既有已核实的调研结论，也有**代码里根本不存在**的拟新增接口，文档开头会写明状态。
...
- **`CLAUDE.md`（仓库根）** —— 架构约定与开发须知：四条管线的分工、`app.py` 组装根的注入顺序、数据库演进方式、打包模式差异、测试写法。改代码前读它。
