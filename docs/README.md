...
| [`assets/`](assets/) | 图片资源：`images/` 供文档内引用（已入库）；[`diagrams/`](assets/diagrams/) 是根 README 的七张架构 / 流程 / 状态机图 —— HTML 图源加渲染出的深浅两版 PNG，改完用同目录的 `render.py` 重渲；[`badges/`](assets/badges/) 是两份 README 的徽章与国旗，事实源是 `scripts/build_readme_badges.py`；`ui-baseline/` 是 UI 回归基线截图（**未入库**，被 `.gitignore` 排除，只在本地存在） | 一般不用直接翻，由各文档引用 |
