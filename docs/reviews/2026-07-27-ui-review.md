...
| 全库无 CRS 字样 | `grep -rn "EPSG\|CRS\|坐标系\|WGS\|GCJ" templates/ static/js/` 结果为空。后端实际用了三套：`download_engine.py:836` 输出标 EPSG:4326、`contour_engine.py:461` warp 到 EPSG:3857、`cesium_terrain.py` 输出 WGS84 椭球 |
...
| 2. 竞品对标 | 检索 SAS.Planet、水经注、QGIS、Global Mapper、cesium、ArcGIS Pro 的界面惯例，提取共性要素清单 |
