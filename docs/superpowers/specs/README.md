...
- **最危险的不是整篇过时，是局部失效**——文件头写着「已实现」，正文里却藏着一节早被推翻的约束。整篇过时的文档会自曝（照做立刻失败），局部失效的会骗人（读起来处处合理，只有那一节是反的）。下面三份点名了。
...
| `2026-08-04-terrain-triangulation-design.md` | 2026-08-04 | **已实施（2026-08-04/05，`f7c0466b2..856812eb8`），但四条核心结论被执行期实测推翻（局部失效，见下）**。落地形态：三角化默认 `'auto'` 逐瓦片择优（**不是**正文写的 `'martini'`）、逐顶点法线默认开、K 固定 0.15、UI 只有 Cesium `enableLighting` 一个开关（默认关）。正文大半是**实测数据与选型排除依据**（3518 个真实瓦片样本，山地 + 平缓两套 DEM）——QEM/PyMeshLab、fast-simplification、自研 TVD + Numba 三条路都实测后排除，附录 B 记了 8 个产生过假数字的坑，这些**仍然有效**。**两条必读**：①「插曲」一节记录了 `triangleCount` 字段写错导致**地形从未真正工作过**，以及测试为何镜像了这个 bug；② 实测同屏峰值仅 27.8 万三角形，**几何不是 GPU 瓶颈**（这条成立；同一段里「减面的价值在存储」那句已被推翻，见下） |
...
**`2026-08-04-terrain-triangulation-design.md`** —— 头部归档状态头列全了，这里只放索引。它是**动工前**写的，执行期的实测推翻了四条核心结论，其中两条的方向是反的：

- `:161`「减面的价值主要在存储与传输（瓦片体积降 74–83%）」—— **那是未压缩字节，而瓦片是 gzip 落盘、gzip 上线**。按落盘字节实测（112,584 张配对瓦片）：山地 **+17.6%**、丘陵 **+9.8%**、平缓 −54.2%，全局加权仅 −9.7%。最终方案因此改成**逐瓦片择优（`triangulator='auto'`）**，不是正文写的 `'martini'`；
- `:194`「ghost cells 100% 归零」—— **只在不发生降采样的最深层成立**（该前提要求采样器是位置的纯函数，生产的 `DemSampler.sample` 不是）。中间层只是压小：z11 边界法线夹角中位 2.42°→0.238°；
- `:113`「K > 0.25 只增加误差、不减少三角形」—— **对山地/丘陵不成立**（山地 K=0.25→0.40 三角形还降 19%）。真实机制是 U 形，不是饱和；
- `:51`「parentUrl 语义未验证，**很可能是对的**」—— **验证了，是错的**。带 `/layer.json` 会让整个 provider 静默降级成 heightmap，4154 m 的山峰被解成海平面以下 744 m。已修（`856812eb8`），但**存量瓦片的 `layer.json` 里仍是坏值，必须重切**。

执行期的全部实测数据与裁定原文在 `../../../.superpowers/sdd/2026-08-04-terrain-triangulation/progress.md`。**仍成立且是唯一记录的**：QEM / fast-simplification / TVD 三条路的排除依据、附录 B 的 8 个统计陷阱、「同屏峰值 27.8 万三角形，几何不是 GPU 瓶颈」、pin_border 是跨瓦片一致性的唯一保证。
