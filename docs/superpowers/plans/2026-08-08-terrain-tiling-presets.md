...
- `speed`: 实际层级 **11**，张数约 893，体积约 2.2 MB

**张数比 ≠ 体积比，别拿张数当判据。** z0–z4 那 682 张（2+8+32+128+512）是全球覆盖的固定底座，三档逐层完全相同、不随档位缩水；真正随 `level_offset` 变的只有塔尖（实测 z11=144 / z12=552 / z13=2162）。所以体积逐档约 1/3.3，张数却只有 3607→1445→893。此处早先写的「约 580」是拿 1445÷2.5 外推的，外推时把那 682 张底座也一起除了 —— 实测是 893，不是缺陷。
