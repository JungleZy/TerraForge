...
(function () {
    ...
    //
    // 栈序 = **注册顺序**，topLayer() 从后往前找第一个 isOpen()，与 z 令牌
    // **无关**。改前这里写的是「静态层在解析期按 z 令牌由低到高注册」，与令牌值
    // 直接矛盾：静态注册序是 面板(--z-panel 1401) → cmdk(--z-cmdk 13100) →
    // 拖拽遮罩(--z-drop-veil 13000)，最后一步 z 是往**下**走的。而
    // confirm / progress(--z-confirm 12000) 是每次打开现注册、节点移出文档时
    // 注销，于是**后召唤的总在栈顶** —— 这正是「最上层」的直觉（刚弹出来挡在
    // 眼前的那一个），可它的 z 恰恰是顶部四层里最低的。按那条不成立的规则加层，
    // 加出来的就是「看着 A、Esc 关掉 B」。
    //
    // 顶部三层的 z 与栈序本来就对不上，摆平它的不是重排 z，而是**让拖拽遮罩在
    // 对话框在场时根本不出场**（drop_process.js 的 dragenter 守卫：topName()
    // 报出白名单外的层、或 body.modal-open，就整个不接管拖拽）。加新层时照这条
    // 办：z 高低随便挑，但必须保证「同时在场的两层里，视觉在上的那个也在栈顶」。
    //
