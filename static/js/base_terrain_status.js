...
(function () {
    ...
    //
    // ⚠️ **与服务端语义的耦合，改任一侧前先读这段。**
    // base_terrain_warmup.start_warmup 的 docstring 明确写着「phase == 'failed' 时会
    // 重新尝试」—— 服务端**保留了** failed → running 这条合法路径。今天它挡不到真
    // 东西：start_warmup 只有 app_factory 一个调用点，一次启动只跑一次。但只要有人
    // 加第二个调用点（界面上的「重试」按钮、切片前的兜底重试），合法的 failed →
    // running 就可达，而这个守卫会把它**永久**挡在外面：用户点了重试，后台真的在
    // 解压，界面却纹丝不动、仍显示「底图不可用」，且不会有任何报错。
    // 那时的改法不是删掉守卫（跨线程窗口依然在），而是让服务端在重试时带一个能区分
    // 「新一轮」的标记（递增的 attempt 序号之类），客户端见到新序号就重置 settled。
    let settled = false;
