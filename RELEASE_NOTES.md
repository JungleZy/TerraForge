...
**四个首发插件**
- **天地图数据源**：影像（`img_w`）与注记（`cia_w`）两个 WMTS 源，开启后出现在下载弹窗的数据源下拉里。需要在插件配置里填自己的 key（`https://console.tianditu.gov.cn/` 申请），填的是 `{"token": "<你的 key>"}`。key **只存在插件配置里**，不进任务行、不进指纹、不进日志——换 key 不会让已下载的瓦片失效。
...
- **GeoPackage 导出**：把已完成任务的栅格产物导出成 `.gpkg`，落在源产物的同级目录。
...
3. **缺 key 时的表现是「全挂」而不是「缺凭据」。** token 没填，天地图返回 401，界面上只看到瓦片下载失败。`GET /api/plugins/sources` 已经返回 `needs_credential` 字段，但前端还没用它做提示。
...
2. **天地图真 key 验一张瓦片**：插件面板开启 `tianditu`，配置填 `{"token": "<真 key>"}`，框一小块区域按天地图影像源下一个 z10 任务。要确认的是：瓦片真的下下来（不是 401、不是配额错误）、图面对得上位置（`TILEROW`/`TILECOL` 没写反）、日志里**没有** token 明文。
