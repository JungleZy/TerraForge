...
REM 版本来源（单一事实源）：src/core/config.py 的 Config.APP_VERSION；也可用第一个参数覆盖：
REM   push-release.bat            用 src/core/config.py 里的版本
...
set VERSION=%~1
if "%VERSION%"=="" (
    REM src/core/config.py 中该行为类属性（带缩进）：APP_VERSION = 'x.y.z'
    for /f "tokens=2 delims='" %%v in ('findstr /r /c:"^ *APP_VERSION *= *'" src/core/config.py') do set VERSION=%%v
)
if "%VERSION%"=="" (
